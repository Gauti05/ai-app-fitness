...
const Setup = () => {
      ...
      try {
        ...
        if (res.data) {
          ...
          setFormData({
            ...
            goal: res.data.goal || 'Muscle Gain',
        ...
        toast.error("Could not load your current stats.");
      } finally {
  ...
  // Show loader while fetching current stats
  if (loadingData) {
    return (
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white font-sans flex items-center justify-center p-4 relative overflow-hidden">
      ...
      {/* Backgrounds */}
      <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=1470&auto=format&fit=crop')] bg-cover bg-center fixed"></div>
        ...
        <div className="md:w-1/3 bg-slate-900/80 p-8 flex flex-col justify-between border-r border-white/10">
          <div>
            ...
            <p className="text-slate-300 text-sm leading-relaxed">
              Keep your stats current. Changing your weight, goal, or activity level will help our AI recalibrate your plan for better results.
            </p>
          </div>
          
          <div className="mt-8 space-y-4 opacity-50">
            <div className="flex items-center gap-3 text-sm text-slate-300">
              <User size={16} /> <span>Current Age: {formData.age}</span>
            </div>
            <div className="flex items-center gap-3 text-sm text-slate-300">
              <Weight size={16} /> <span>Current Weight: {formData.weight}kg</span>
            </div>
            <div className="flex items-center gap-3 text-sm text-slate-300">
              <Target size={16} /> <span>Goal: {formData.goal}</span>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-1.5">
                ...
                <input type="number" name="age" value={formData.age} onChange={handleChange} className="w-full bg-slate-950/50 border border-slate-700 rounded-lg py-3 px-4 text-white focus:border-teal-400 outline-none transition" required />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input type="number" name="weight" value={formData.weight} onChange={handleChange} className="w-full bg-slate-950/50 border border-slate-700 rounded-lg py-3 px-4 pl-10 text-white focus:border-teal-400 outline-none" required />
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input type="number" name="height" value={formData.height} onChange={handleChange} className="w-full bg-slate-950/50 border border-slate-700 rounded-lg py-3 px-4 pl-10 text-white focus:border-teal-400 outline-none" required />
